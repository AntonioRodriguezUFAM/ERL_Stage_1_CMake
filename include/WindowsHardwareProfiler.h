...
public:
    std::string profileHardware() override {
        ...
        if (std::system(command.c_str()) != 0) {
            std::cerr << "Failed to start WPR session." << std::endl;
            return "";
        }
        ...
        if (std::system(command.c_str()) != 0) {
            std::cerr << "Failed to stop WPR session." << std::endl;
            return "";
        }
        ...
        if (std::system(command.c_str()) != 0) {
            std::cerr << "Failed to export WPR data." << std::endl;
            return "";
        }

        std::string reportData = parseWPRData("profile_report.txt");

        // Clean up temporary files
        std::remove("profile.etl");
        std::remove("profile_report.txt");
